...
namespace canvas {
	...
	void Canvas::calculateSolutions(int linkage_type, int num_samples, std::vector<std::pair<double, double>>& sigmas, bool avoid_branch_defect, bool rotatable_crank, double position_error_weight, double orientation_error_weight, double linkage_location_weight, double trajectory_weight, double size_weight) {
		...
		if (linkage_region_pts.size() < poses.size()) {
			...
			for (int i = num_linkage_regions; i < poses.size(); i++) {
				linkage_region_pts[i].push_back(glm::dvec2(30, 0));
				linkage_region_pts[i].push_back(glm::dvec2(30, 30));
				linkage_region_pts[i].push_back(glm::dvec2(60, 30));
				linkage_region_pts[i].push_back(glm::dvec2(60, 0));
			}
		}
		...
		else if (solutions[0].size() == 1) {
			mainWin->ui.statusBar->showMessage("1 candidate was found.");
		}
		else {
			mainWin->ui.statusBar->showMessage(QString("%1 candidates were found.").arg(solutions[0].size()));
		}
	...
	void Canvas::updateDefectFlag(const std::vector<std::vector<glm::dmat3x3>>& poses, const kinematics::Kinematics& kinematics) {
		/*
			...
			if (mode == MODE_MOVE) {
				...
				if (selectedJoint.first >= 0) {
					...
					if (ctrlPressed) {
						...
						updateDefectFlag(poses, kinematics[linkage_id]);
					}
					else {
						// select a solution
						glm::dvec2 pt = screenToWorldCoordinates(e->x(), e->y());
					...
					// update the geometry
					synthesis->updateBodies(kinematics[0], body_pts);
